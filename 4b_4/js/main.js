  ...
  _createClass(Datavis, [{
    ...
    value: function render() {
      ...
      return function () {
        ...
        _elem4.setAttribute('class', 'map-item');

        _elem4.appendChild(document.createTextNode('\n            '));

        var _expr = _this2.options.map(function (option) {
          var className = 'map-item-image ' + (option === _this2.active ? '-active' : '');
          return function () {
            var _elem6 = document.createElement('div');

            _elem6.setAttribute('class', className);

            _elem6.setAttribute('style', 'background-image: url(assets/' + option + '_4.jpg)');
        ...
        var _expr2 = YearSelector({ options: _this2.options, title: "select a year", active: _this2.active }),
