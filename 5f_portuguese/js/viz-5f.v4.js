...
const DEFAULT_SELECTION = ["AMAGGI","LOUIS DREYFUS","AMAGGI & LD COMMODITIES"];
...
const DEFAULT_RADIUS_RANGE = [3, 35];
...
const DEFAULT_CATEGORIZED_COLOR_SCALE = ["#EDCE4A","#719DE0","#D66ED9"];
...
function MapsViz(amount, map_render, map, geo_data, aux_data, default_source, year_range, default_selection, colors) {
  this.amount = amount;
  this.map = map;
...
MapsViz.prototype.enableLegend = function (left_range, right_range) {
  ...
  renderMapLegend(`${this.map_prefix}map2`, this.left_legend_range, this.right_legend_range[this.data_source]);
}
...
MapsViz.prototype.enableAuxDataChartSelector = function (selector, ranges) {
  ...
  this.aux_chart_selector.initChart(this.selector_data, this.current_companies_selection, DEFAULT_CATEGORIZED_COLOR_SCALE);
}
...
(function () {
  Promise.all([
    ...
    viz.enableLegend(
      ...
      {
        absolute: {title: 'RISCO DE DESMATAMENTO', range: absolute_risk_range, unit: "HA"},
        relative: {title: 'RISCO DE DESMATAMENTO', range: relative_risk_range, unit: "HA/KT"}
      });
